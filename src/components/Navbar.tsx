...
const Navbar = () => {
  ...
  return <nav className="bg-[#259e63] text-white dark:bg-[#1e3a2d] shadow-md sticky top-0 z-50">
      ...
      {isMenuOpen && <div className="md:hidden">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3 bg-[#1e3a2d]">
            ...
            <Link to="/skills" className="text-white hover:bg-[#259e63] block px-3 py-2 rounded-md" onClick={() => setIsMenuOpen(false)}>
              Thông tin kỹ năng
            </Link>
            <Button variant="ghost" size="icon" onClick={toggleDarkMode} className="text-white hover:bg-[#1e3a2d] hover:text-white">
              {isDarkMode ? <Sun size={20} /> : <Moon size={20} />}
